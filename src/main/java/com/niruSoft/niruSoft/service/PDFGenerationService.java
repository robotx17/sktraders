...
public class PDFGenerationService implements PDFGenerationServiceImpl {
    ...
    public CompletableFuture<PDFData> generatePDFFromJSONAsync(String jsonData, String farmerName, String date) throws IOException {
        ...
        ClassPathResource fontResource = new ClassPathResource("static/ARIAL.ttf");
        PdfFont font = PdfFontFactory.createFont(fontResource.getContentAsByteArray(), PdfEncodings.IDENTITY_H);
        ...
        try {
            ...
            return CompletableFuture.supplyAsync(() -> {
                try (ByteArrayOutputStream pdfOutputStream = new ByteArrayOutputStream(); PdfWriter pdfWriter = new PdfWriter(pdfOutputStream); PdfDocument pdfDocument = new PdfDocument(pdfWriter)) {
                    ...
                    String imagePath = "https://raw.githubusercontent.com/Santosh2740/images/main/SKTRADER.jpg";
                    ImageData imageData = ImageDataFactory.create(imagePath);
                    Image image = new Image(imageData);
//                    ClassPathResource imageResource = new ClassPathResource("static/SKTRADER.jpg");
...
//                    Image image = new Image(imageData);
                    ...
                    for (Map<String, String> bagsumDetails : bagsumDetailsList) {
                        ...
                        Cell slNoCell = new Cell().add(new Paragraph(serialNumber)).setTextAlignment(TextAlignment.CENTER).setFontSize(8);
...
//                    System.out.println(heightFromStartToLastDataRow);
                    ...
                    Paragraph paragraphTotla = new Paragraph(strTotalSumKgBAg + "  (Kg/Bags/Pkt/Box/Crate)").setMarginTop(5).setMarginLeft(130).setFontSize(8);
                    ...
                    Paragraph totalParagraph = new Paragraph().setFontSize(16).add("Total:   ").setMarginRight(5).add(new Text(currencySymbol)).add(new Text(totalAsString)).setFont(font).setBold().setFontColor(brownColor).add("\n").setTextAlignment(TextAlignment.RIGHT);
//                    Paragraph totalParagraph = new Paragraph().setFontSize(16).add("Total:     ").setMarginRight(5).add(new Text(currencySymbol)).add(new Text(totalAsString).setBold()).setFontColor(brownColor).add("\n").setTextAlignment(TextAlignment.RIGHT);
    ...
    public CompletableFuture<PDFData> generatePDFFromJSONAsyncSecond(String jsonData, String farmerName, String date) throws IOException {
        ...
        ArrayNode items = (ArrayNode) jsonNode.get("ITEM");
        ArrayNode qty = (ArrayNode) jsonNode.get("QTY");
        ArrayNode rate = (ArrayNode) jsonNode.get("Rate");
        ArrayNode count = (ArrayNode) jsonNode.get("COUNT");
        ArrayNode amount = (ArrayNode) jsonNode.get("AMT");
        DecimalFormat df = new DecimalFormat("#,###");

        List<ItemData> itemDataList = IntStream.range(0, items.size())
                .mapToObj(i -> new ItemData(
                        ...
                        count.get(i).asInt(),
                        amount.get(i).asInt()
                )).sorted(Comparator.comparingInt(ItemData::getQty).reversed()).toList();

        int amountSum = itemDataList.stream().mapToInt(ItemData::getAmount).sum();
        String formattedAmountSum = df.format(amountSum);
        ...
        try {
            ...
            return CompletableFuture.supplyAsync(() -> {
                try (ByteArrayOutputStream pdfOutputStream = new ByteArrayOutputStream(); PdfWriter pdfWriter = new PdfWriter(pdfOutputStream); PdfDocument pdfDocument = new PdfDocument(pdfWriter)) {
                    pdfDocument.addEventHandler(PdfDocumentEvent.END_PAGE, new AddressEventHandler());
                    pdfDocument.addEventHandler(PdfDocumentEvent.END_PAGE, new PageNumberEventHandlerSecond());

                    pdfDocument.addEventHandler(PdfDocumentEvent.START_PAGE, new HandlerHeaderLogoS());

                    DateHandler datesHandler = new DateHandler(date);
                    pdfDocument.addEventHandler(PdfDocumentEvent.START_PAGE, datesHandler);

                    HeadersHandler headersHandler = new HeadersHandler(farmerName);
                    pdfDocument.addEventHandler(PdfDocumentEvent.START_PAGE, headersHandler);
                    ...
                    Table table = new Table(UnitValue.createPercentArray(new float[]{40, 20, 20, 20, 20}));
                    table.setWidth(UnitValue.createPercentValue(100));
                    table.setFontSize(19);

                    document.setTopMargin(110);


                    String[] headers = {"ITEMS", "QTY", "RATE", "COUNT", "AMOUNT"};
                    for (String headerText : headers) {
                        Cell headerCell = new Cell().add(new Paragraph(headerText));
                        headerCell.setTextAlignment(TextAlignment.CENTER).setBold();
                        table.addHeaderCell(headerCell);
                    }

                    for (ItemData itemData : itemDataList) {
                        table.addCell(new Cell().add(new Paragraph(itemData.getItem()).setTextAlignment(TextAlignment.LEFT).setFontSize(14)));
                        table.addCell(new Cell().add(new Paragraph().setTextAlignment(TextAlignment.CENTER).add(itemData.getQty() == 0 ? "-" : String.valueOf(itemData.getQty()))));
                        table.addCell(new Cell().add(new Paragraph().setTextAlignment(TextAlignment.CENTER).add(itemData.getRate() == 0 ? "-" : String.valueOf(itemData.getRate()))));
                        table.addCell(new Cell().add(new Paragraph().setTextAlignment(TextAlignment.CENTER).add(itemData.getCount() == 0 ? " " : String.valueOf(itemData.getCount()))));
                        table.addCell(new Cell().add(new Paragraph().setTextAlignment(TextAlignment.RIGHT).add(new Text(currencySymbol).setFont(font)).add(String.valueOf(itemData.getAmount()))));
                    }

                    Cell totalAmountCell = new Cell(1, 5);

                    Paragraph totalAmountParagraph = new Paragraph();
                    totalAmountParagraph.addTabStops(new TabStop(500f, TabAlignment.LEFT));
                    totalAmountParagraph.add("Grand Total ");
                    totalAmountParagraph.setBold();
//                    totalAmountParagraph.setMarginLeft(6);
                    totalAmountParagraph.setMarginTop(4);
                    Paragraph valueParagraph = new Paragraph()
                            .add(new Text(currencySymbol).setFont(font))
                            .add(" " + formattedAmountSum)
                            .setTextAlignment(TextAlignment.RIGHT);
                    valueParagraph.setFontColor(DeviceRgb.BLUE);
                    valueParagraph.setMarginTop(-30);
//                    valueParagraph.setMarginRight(14);
                    valueParagraph.setBold();
                    totalAmountCell.add(totalAmountParagraph);
                    totalAmountCell.add(valueParagraph);
